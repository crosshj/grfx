...
import { isInitedFactory, isNumeric } from './utils.js';

function appendCss() {}

function sidebar() {
	return returnProps(sidebar);
}
sidebar.scripts = [];
	...
	function sidebarStart({ sidebarDef }, startCallback) {
		const getRoot = (components, dispatcher) => {
			...
			function selectComponent({
				...
				const currentValue = (globalState.find((x) => x.key === key) || {})
					.value;
				const value = currentValue || item.default;
			...
			function layersComponent({
				...
				const handleReOrder = ({ item, layers, draggedItem, dropTarget }) => {
					var order = layers.map((x, i) =>
						isNumeric(x.number) ? x.number : i
					);
				...
				function dragStartHandler(layersIndex, e) {
					...
					document
						.querySelector(".layers ul")
						.classList.add("contains-dragging");
		...
		const getInitialState = () => ({
			layersHidden: sidebarDef.sections[0].items.find(
				(x) => x.type === "layers"
			).layersHidden,
			globalState: [],
			layersProperties: [],
